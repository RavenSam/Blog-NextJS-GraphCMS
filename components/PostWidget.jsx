...
export default function PostWidget({ categories, slug }) {
   ...
   return (
      <div className="bg-white shadow-lg rounded-lg p-8 mb-8  ">
         <h3 className="text-xl mb-8 font-semibold border-b pb-6 ">{slug ? "Related Posts" : "Recent Posts"}</h3>
            ...
            <div key={index} className="flex items-center w-full mb-4">
               ...
               <div className="flex-grow ml-4">
                  ...
                  <Link href={`/post/${post.slug}`}>
                     <a className=" text-sm block transition  duration-200  hover:text-pink-600 ">{post.title}</a>
                  </Link>
               </div>
            </div>
         ))}
